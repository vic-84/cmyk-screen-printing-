...
def load_document(path, dpi=300, page=0):
    ...
    if ext == '.eps':
        return _load_eps(path, dpi)
    return _load_raster(path)


def _to_srgb(image, notes):
    ...
    if image.mode == 'CMYK':
        source = ImageCms.ImageCmsProfile(io.BytesIO(icc)) if icc else ImageCms.getOpenProfile(DEFAULT_CMYK_PROFILE)
        notes.append("CMYK convertido a RGB con " + ("su perfil ICC" if icc else "GRACoL 2006 (sin perfil incrustado)"))
...
def _load_psd(path):
    ...
    array = _to_srgb(composite, notes)
    dpi = 72.0
    try:
